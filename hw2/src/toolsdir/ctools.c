...

{
    ...
    {
        while (*s1 != '\0')
            if (0 == nocase_compare(s1++, len2, s2, len2))
                return (s1 - 1);
    }
    else
    {
...

{
    ...
    if (1 >= (len = strlen(oldstring)) && pretrim)
        return (len);
    if (posttrim)
    {
        np = oldstring + len - 1;
...
int str_to_pos_int(astring, low, high) char *astring;
int low, high;
...

{
    ...
    len = strlen(astring);
    if (!all_digits(astring))
        return (-1);
    if (len > maxlen)
        return (-2);
    if (len == maxlen)
    {
        if (1 == strcmp(astring, MAXINTSTR))
            return (-2);
    }
    for (j = 0; j < len - 1; j++)
    {
...
char *temp_path(dir, filename) char *dir;
char *filename;
...
char *perm_path(dir, filename) char *dir;
char *filename;
...
char *just_filename(path, new, perm) char *path;
Bool new, perm;
...
int read_yes_or_no(FILE *iport, FILE *oport, char *prompt, char *helpstring, char *quitstring)
{
    ...
    while (T)
    {
        ...
        case (TOO_MANY_CHARS):
            break;
        default:
            if (0 == (bl = remove_excess_blanks(buffer2, buffer)))
                break;
...
extern char *efopen(filename, mode) char *filename;
char *mode;
...
Bool check_string(s, minlen, maxlen) char *s;
long minlen, maxlen;
{
    long len;
    if (s == 0)
