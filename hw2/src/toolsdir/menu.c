...
try_again:
    ...
    for (j = 0; j < n_options; j++)
    {
        ...
        if (!isub && len != optionlen)
            continue;

        /* use different comparision functions depending on whether case */
        /* is important or not. */

        compare = casetest ? strncmp(sline, options[j], len) : nocase_compare(sline, len, options[j], len);
